...
@section('content')
                    ...
                    @if (Auth::user()->user_type_id == 4 || Auth::user()->user_type_id == 5)
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><button type="button" class="btn btn-block btn-primary btn-sm"
                                    data-toggle="modal" data-target="#Talukusers"><i class="fa fa-plus"> Add </i></button>
                            </li>
                        </ol>
                    @else
                    @endif
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive" style="overflow-x: auto; ">
                                <table id="example2" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($talukusers as $key => $talukusers)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>N{{ $talukusers->userID }}</td>
                                                <td>{{ $talukusers->district_name }}</td>
                                                <td>{{ $talukusers->taluk_name }}</td>
                                                <td>{{ $talukusers->full_name }}</td>
                                                <td>{{ $talukusers->phone }}</td>
                                                <td>{{ $talukusers->status }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu" role="menu">
                                                            <a class="dropdown-item"
                                                                href="{{ url('/edituser', $talukusers->userID) }}">Edit
                                                                User</a>
                                                            <a onclick="userdatas('{{ $talukusers->userID }}','{{ $talukusers->full_name }}','{{ $talukusers->username }}','{{ $talukusers->district_name }}','{{ $talukusers->taluk_name }}','{{ $talukusers->email }}','{{ $talukusers->pas }}','{{ $talukusers->phone }}','{{ $talukusers->status }}','{{ $talukusers->user_photo }}',)" type="button" class="dropdown-item">View User</a>


                                                            @if (Auth::user()->user_type_id == 1 || Auth::user()->user_type_id == 2 || Auth::user()->user_type_id == 3)
                                                                <a onclick="userstatus('{{ $talukusers->userID }}','{{ $talukusers->full_name }}','{{ $talukusers->from_to_date }}','{{ $talukusers->status }}')"
                                                                    type="button" class="dropdown-item">Status</a>
                         <a href="resetpassword/{{ $talukusers->userID }}" type="button" class="dropdown-item">Reset Password</a>

                                                            @else
                                                                <a class="dropdown-item"
                                                                    href="{{ url('/userstatusupdate') }}/{{ $talukusers->userID }}/{{ $talukusers->user_type_id }}">Status
                                                                    Update</a>
                                                            @endif

                                                            <a onclick="userwallet('{{ $talukusers->userID }}','{{ $talukusers->full_name }}','{{ $talukusers->balance }}')"
    ...
    <div class="modal fade" id="Talukusers">
...
@push('page_scripts')
    <script>
        function userdatas(id, full_name, username, district_name, taluk_name, email, pas, phone, status) {
            $("#id").text(id);
