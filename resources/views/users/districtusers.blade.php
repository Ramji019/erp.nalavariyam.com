                    ...
                    @if (Auth::user()->user_type_id == 2 || Auth::user()->user_type_id == 3)
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><button type="button" class="btn btn-block btn-primary btn-sm"
                                    data-toggle="modal" data-target="#districtusers"><i class="fa fa-plus"> Add </i></button>
                            </li>
                        @else
                    @endif
                </div>
                                ...
                                <table id="example2" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($districtusers as $key => $districtuserslist)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>N{{ $districtuserslist->userID }}</td>

                                                <td>{{ $districtuserslist->district_name }}</td>
                                                @if (Auth::user()->user_type_id == 1)
                                                    <td>{{ $districtuserslist->group_name }}</td>
                                                @endif
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu" role="menu">
                                                            <a class="dropdown-item"
                                                                href="{{ url('/edituser', $districtuserslist->userID) }}">Edit
                                                                User</a>

                                                            <a onclick="userdatas('{{ $districtuserslist->id }}','{{ $districtuserslist->full_name }}','{{ $districtuserslist->user_photo }}','{{ $districtuserslist->username }}','{{ $districtuserslist->district_name }}','{{ $districtuserslist->email }}','{{ $districtuserslist->pas }}','{{ $districtuserslist->phone }}','{{ $districtuserslist->status }}')" type="button" class="dropdown-item">View User</a>
@if(Auth::user()->user_type_id == 1 || Auth::user()->user_type_id == 2 || Auth::user()->user_type_id == 3)
                         <a onclick="userstatus('{{ $districtuserslist->userID }}','{{ $districtuserslist->full_name }}','{{ $districtuserslist->from_to_date }}','{{ $districtuserslist->status }}')" type="button" class="dropdown-item">Status</a>
                         <a href="resetpassword/{{ $districtuserslist->userID }}" type="button" class="dropdown-item">Reset Password</a>

@else 

                    <a class="dropdown-item" href="{{ url('/userstatusupdate') }}/{{ $districtuserslist->userID }}/{{ $districtuserslist->user_type_id }}">Status Update</a>

               
@endif

                                                            <a onclick="userwallet('{{ $districtuserslist->userID }}','{{ $districtuserslist->full_name }}','{{ $districtuserslist->balance }}')" type="button" class="dropdown-item">View Wallet</a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
