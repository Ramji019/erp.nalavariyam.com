                        ...
                        <tbody>
                            @foreach ($paymentrequest as $key => $payreq)
                                <tr>
                                    ...
                                    <td>{{ $payreq->status }}</td>
								    @elseif(Auth::user()->id == $payreq->to_id)
									 @if ($balance < $payreq->amount)
										 <td><a class="btn btn-info btn-sm" href="#" onclick="reqamount('{{ $payreq->id }}','{{ $payreq->amount }}','{{ $payreq->from_id }}','{{ $payreq->req_image }}')">Request</a></td>
										 @else
											 
									<td><a class="btn btn-success btn-sm" href="#" onclick="show_reqamount('{{ $payreq->id }}','{{ $payreq->amount }}','{{ $payreq->from_id }}','{{ $payreq->req_image }}')">Appprove</a></td>
                                   @endif
									@endif
    ...
    <div class="modal fade" id="requestamount">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ url('requestamount_approve') }}" method="post" class="form-horizontal" id="formABC">
                    {{ csrf_field() }}
                    <div class="modal-body">
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <input type="submit" class="btn btn-primary" value="Add Payment" name="addamount" id="requestamount" />
                    </div>
                </form>
            </div>
        </div>
